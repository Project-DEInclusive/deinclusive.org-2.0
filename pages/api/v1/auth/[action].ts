    ...
    switch (action) {
        case "myinfo":
            ...
            if (authresp.success && authresp.user.id.length > 0) {
                let resp = await User.findOne({ _id: authresp.user.id, draft: false, deleted: false }, { name: 1, status: 1, "thumbnail.src": 1, email: 1 });
                if (resp) {
...
async function handleSignin(req: NextApiRequest, res: NextApiResponse) {
    await mongodb.Connect();
    try {
        let resp = await User.findOne({ authid: req.body.authid, draft: false, deleted: false }, { name: 1, status: 1, "thumbnail.src": 1, email: 1 });
        if (resp) {
            apiauthresolver.setToken(res, { id: resp._id, name: `${resp.name.first} ${resp.name.last}`, status: resp.status }); // Set the token
            return res.status(200).json({ user: resp });
        } else {
...
async function handleSignup(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const resp = await User.create({ ...user, authid: authid, draft: req.query.draft === "true" }); // Create the user
        if (resp) {
            apiauthresolver.setToken(res, { id: resp._id, name: `${resp.name.first} ${resp.name.last}`, status: resp.status }); // Set the token
        }
        const userobj = p.resolveObjectArray(resp, p.create.hiddenprops);
        return res.status(200).json({ user: { _id: userobj._id, name: userobj.name, status: userobj.status, "thumbnail.src": userobj.thumbnail.src } }); // Return the sanitized user
    } catch (e: any) {
        return res.status(500).json({ error: e?.message });
    }
}
