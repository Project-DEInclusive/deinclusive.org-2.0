...
import Schema from "../../../../types/schema/recruit/SRecruit";
import Event from "../../../../types/schema/event/SEvent";
...
async function handleGET(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        let count = 0;
        let resp = {};
        var p = permissions.getUserCollectionPermission(userType, "recruit"); // Get the permissions for the current obj
...
async function handlePOST(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const event = await Event.findById(obj.eventid, { _id: 1, name: 1, owner: 1 }); // Get the event name
        if (!event) return res.status(404).json({ error: "Event not found" }); // Check if the event exists
        console.log(event);
        obj.requester.id = userId; // Set the requester id
        obj.eventname = event.name; // Set the event name
        obj.eventownerid = event.owner; // Set the event owner
