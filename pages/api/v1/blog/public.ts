...
import Schema from "../../../../types/schema/blog/SBlog";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    const authresp = apiauthresolver.resolveToken(req, res); // Resolve the blog type & id4
    if (authresp.success) {
...
async function handleGET(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        let count = 0;
        let resp = {};
        var p = permissions.getUserCollectionPermission(userType, "blogpublic"); // Get the permissions for the current blog
        if (!p.read.allowed) return res.status(403).json({ error: "Forbidden" }); // Check if the blog has the permission to read the collection
