...
import Schema from "../../../../types/schema/event/SEvent";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    const authresp = apiauthresolver.resolveToken(req, res); // Resolve the event type & id4
    if (authresp.success) {
...
async function handleGET(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        let count = 0;
        let resp = {};
        var p = permissions.getUserCollectionPermission(userType, "eventpublic"); // Get the permissions for the current event
        if (!p.read.allowed) return res.status(403).json({ error: "Forbidden" }); // Check if the event has the permission to read the collection
