...
import Schema from "../../../../types/schema/user/SUser";
...
async function handleGET(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        let count = 0;
        let resp = {};
        var p = permissions.getUserCollectionPermission(userType, "userpublic"); // Get the permissions for the current user
        if (!p.read.allowed) return res.status(403).json({ error: "Forbidden" }); // Check if the user has the permission to read the collection
