...
import Schema from "../../../../types/schema/roleverification/SRoleVerification";
...
async function handleGET(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        let count = 0;
        let resp = {};
        var p = permissions.getUserCollectionPermission(userType, "roleverification"); // Get the permissions for the current obj
...
async function handlePATCH(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (obj.status != undefined && obj.status == EApprovalState.approved) {
            const toup = await Schema.find({ ...filter }, { _id: 0, requester: 1, role: 1 });
            toup.forEach(async (requsr) => {
                const respuser = await User.findByIdAndUpdate(requsr.requester.id, { $set: { status: requsr.role } });
            });
        }
