...
// Set create permissions for the 'users' collection for the 'Signed' role
const createPermission = new CActionPermission("create");
createPermission.allowed = false;
createPermission.immutableprops = ["_id", "owner", "status", "draft", "deleted", "jobcount", "eventcount", "authid"]; // Define the properties that can't be updated by 'Signed' role
createPermission.hiddenprops = ["__v", "deleted", "authid"]; // Define the properties dont't need send  to the client

// Set read permissions for the 'users' collection for the 'Signed' role
...
// Set update permissions for the 'users' collection for the 'Signed' role
const updatePermission = new CActionPermission("update");
updatePermission.allowed = true;
updatePermission.immutableprops = ["_id", "owner", "status", "draft", "deleted", "jobcount", "eventcount", "authid"]; // Define the properties that can't be updated by 'Signed' role
...
// Set delete permissions for the 'users' collection for the 'Signed' role
